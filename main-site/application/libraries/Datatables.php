...
ini_set('display_errors', 0);
        ...
        public function select($columns, $backtick_protect = TRUE)
        {
            ...
            {
                if(stristr($val, 'CAST') || stristr($val, 'CONCAT')){
                    $column = trim($val);
                } else {
        ...
        private function get_filtering()
        {
            ...
            if($sSearch != '')
                for($i = 0; $i < count($mColArray); $i++)
                    ...
                    else if ($mColArray[$i]['equals'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns))
                        $sWhere .= $this->select[$this->columns[$i]] . " = '" . $sSearch . "' OR ";
