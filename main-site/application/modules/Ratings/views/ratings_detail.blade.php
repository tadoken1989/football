...
<div class="New_col-centre">
   ...
   <div style="background-color:#e9ece4; text-align:left;">
      <h2 class="bg_h2">BẢNG XẾP {{ $leagues->name }}</h2>
   </div>
   <div class="menu_trd">
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
         <tbody>
            ...
            @if(isset($leagues) && $leagues !== false)
               @if($leagues->is_cup == 0 && $league_team_standing)
                  @foreach($league_team_standing as $key => $ele)
                     <tr class="{{ add_class($ele->league->number_c1,$ele->league->number_c2,$leagues,$key) }}">
                        ...
                        <td class="bxh_gd_1">{{ $ele->played }}</td>
                        <td class="bxh_gd_1">{{ $ele->won }}</td>
                        <td class="bxh_gd_1">{{ $ele->draw }}</td>
                        <td class="bxh_gd_1">{{ $ele->lost }}</td>
                        <td class="bxh_gd_1">{{ $ele->goals_for }}</td>
                        <td class="bxh_gd_1">{{ $ele->goals_against }}</td>
                        <td class="bxh_gd_1">{{ ($ele->goal_difference > 0)?"+".$ele->goal_difference:$ele->goal_difference }}</td>
                        <td class="bxh_gd_1"><b>{{  $ele->points }}</b></td>
                     </tr>
                  @endforeach
               @else
                  @if($league_group_cup)
                     @foreach($league_group_cup as $key => $cup)
                     ...
                     @if(league_team_cup_standing($cup->league_group_id))
   ...
   <div class="mm">
      ...
      @if($leagues->is_cup == 0)
         <a class="C1">&nbsp;</a>&nbsp;Champions League<br>
         <a class="VLC1">&nbsp;</a>&nbsp;VL Champions League<br>
         <a class="C2">&nbsp;</a>&nbsp;UEFA Europa Leage<br>
         <a class="XH">&nbsp;</a>&nbsp;Xuống hạng
      @endif
   </div>
