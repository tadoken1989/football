...
class Imdb
{
    ...
    {
        preg_match('/tt([0-9])+/', $imdbId, $matches);
    ...
    private function scrapeMovieInfo($imdbUrl, $getExtraInfo = true)
    {
        ...
        $arr['title'] = html_entity_decode(str_replace('"', '', trim($this->match('/<title>(IMDb \- )*(.*?) \(.*?<\/title>/ms', $html, 2))), ENT_QUOTES);
        ...
        $arr['release_date'] = '01-01-' . $this->match('/\([^a-zA-Z0-9]*?([0-9]{4})[^a-zA-Z0-9]*?\)[^<]*<\/a>[^<]*<\/div>/ms', $html, 1);
        if ($arr['release_date'] == '01-01-') {
            $arr['release_date'] = '01-01-' . $this->match('/\([^<]*<a[^>]*>[^<]*([0-9]{4})[^<]*<\/a>/ms', $html, 1);
        }
        if ($arr['release_date'] == '01-01-') {
            $arr['release_date'] = '01-01-' . $this->match('/<span[^>]*>[^<]*\([^a-zA-Z0-9]*([0-9]{4})[^a-zA-Z0-9]*\)[^<]*<\/span>/ms', $html, 1);
        }
        if ($arr['release_date'] == '01-01-') {
            $arr['release_date'] = '01-01-' . $this->match('/\([^a-zA-Z0-9]*?([0-9]{4})[^\-0-9a-zA-Z]*\-[^<]*\)[^<]*<\/a>[^<]*<\/div>/ms', $html, 1);
        }
        if ($arr['release_date'] == '01-01-') {
            $arr['release_date'] = '01-01-' . $this->match('/<span[^>]*>[^<]*\([^a-zA-Z0-9]*([0-9]{4})[^\-0-9a-zA-Z]*\-[^<]*\)[^<]*<\/span>/ms', $html, 1);
        }
        //echo $html;
        //preg_match_all('/\([^a-zA-Z0-9]*?([0-9]{4})[^a-zA-Z0-9]*?\)[^<]*<\/a>[^<]*<\/div>/ms', $html, $match);
        //var_dump($match);
        $arr['tagline'] = trim(strip_tags($this->match('/Tagline:<\/h5>.*?<div class="info-content">(.*?)(<a|<\/div)/ms', $html, 1)));
        $arr['plot'] = html_entity_decode(trim(strip_tags($this->match('/Plot:<\/h5>.*?<div class="info-content">(.*?)(<a|<\/div|\|)/ms', $html, 1))), ENT_QUOTES);
    ...
    private function geturl($url, $params = array(), $headers = array(), $timeout = 30)
    {
        $ch = curl_init();
        ...
        if (ENVIRONMENT == 'production') {
            if ($this->sock) {
                curl_setopt($ch, CURLOPT_PROXY, $this->sock);
